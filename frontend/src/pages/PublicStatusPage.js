// frontend/src/pages/PublicStatusPage.js
...
const PublicStatusPage = () => {
  ...
  useEffect(() => {
    const fetchStatusPage = async () => {
      try {
        ...
        try {
          const incidentsRes = await axios.get(`/public/status-pages/${slug}/incidents`);
  ...
  // Get incident impact class
  const getIncidentImpactClass = (impact) => {
    switch (impact) {
      case 'minor':
        return 'bg-info';
      case 'major':
      ...
      <div className="container">
        <div className="row justify-content-center">
          <div className="col-md-8 text-center py-5">
            <LoadingSpinner size="lg" text="Loading status page..." />
          </div>
        </div>
      </div>
    );
  ...
  // Apply theme from status page
  const themeStyles = {
    backgroundColor: statusPage.theme?.backgroundColor || '#f8f9fa',
    color: '#333',
    '--primary-color': statusPage.theme?.primaryColor || '#4CAF50'
  };

  return (
    <div className="public-status-page min-vh-100" style={themeStyles}>
      <div className="container py-4">
        ...
        <header className="text-center mb-5">
          ...
          <h1 className="display-4 fw-bold" style={{ color: 'var(--primary-color)' }}>
            {statusPage.name}
          </h1>
        ...
        {statusPage.resolvedIncidents && statusPage.resolvedIncidents.length > 0 && (
          <div className="mb-5">
            ...
            <div className="accordion" id="resolvedIncidents">
              {statusPage.resolvedIncidents.map((incident, index) => (
                    ...
                    <div className="accordion-body">
                      <div className="timeline">
                        {incident.updates.map((update, updateIndex) => (
                          <div className="timeline-item" key={updateIndex}>
                            <div className="timeline-badge">
                              <i className={`bi ${
                                update.status === 'investigating' ? 'bi-search' :
                                update.status === 'identified' ? 'bi-info-circle' :
        ...
        <footer className="text-center pt-4 pb-5 text-muted">
          <div className="mb-2">
            Powered by <strong style={{ color: 'var(--primary-color)' }}>Monitoring System</strong>
          </div>
          <div>
